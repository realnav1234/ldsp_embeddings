...
def calculate_edi_scores(mutual_info_df, wilcoxon_results_df, rfe_results_df, N=20):
    ...
    WEIGHTS = {"mutual_info": 0.2, "wilcoxon": 0.6, "rfe": 0.2}
    ...
    wilcoxon_scores = (wilcoxon_scores - wilcoxon_scores.min()) / (
        wilcoxon_scores.max() - wilcoxon_scores.min()
    )
