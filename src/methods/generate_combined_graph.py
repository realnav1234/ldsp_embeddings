...
def create_combined_graph(
    mutual_info_df, wilcoxon_results_df, rfe_results_df, results_directory, N=15
):
    """
    ...
    top_N_wilcoxon = wilcoxon_results_df.nsmallest(N, "wilcoxon_pvalue_bh")[
        "dimension"
    ].values
    ...
    plt.bar(
        np.arange(len(mutual_informations)),
        mutual_informations,
        color=bar_colors,
    ...
    plt.scatter(
        rfe_dimensions,
        mutual_informations[rfe_dimensions],
        color="#00C853",
        marker="^",
        label=f"Top {N} (RFE)",
        s=50,
    ...
    if triple_overlap:
        plt.scatter(
            list(triple_overlap),
            mutual_informations[list(triple_overlap)],
            facecolors="none",
            edgecolors="red",
            s=200,
            linewidth=2,
            label="Top 25 in All 3 Analyses",
            marker="o",
        )
...
if __name__ == "__main__":
    ...
    for embeddings_csv in tqdm(embedding_filepaths):
        ...
        wilcoxon_results_df = pd.read_csv(
            os.path.join(
                get_results_directory(
        ...
        create_combined_graph(
            mutual_info_df, wilcoxon_results_df, rfe_results_df, results_directory, N=25
        )
